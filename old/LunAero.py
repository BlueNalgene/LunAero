...
"""
LunAero.py code written by Wesley T. Honeycutt wes.honeycutt@okstate.edu
GitHub Repo here (TBA)

This is the python code for controlling the motor controllers and object
recognition based on OpenCV.  The code will automatically track a high
contrast light source (such as the moon against the night sky) and count 
silhouettes crossing the light source.
Variables are found in config.py and initialization code is found in init.py

Some of this code is based on speed-cam.py written by Claude Pageau (pageauc@gmail.com)
...
Install from a terminal per commands below.
Code should run on a non RPI platform using a Web Cam
"""

print("Loading Please Wait ....")
print("-------------------------------------------------------------------------------------------------")
print("LunAero.py %s using python2 and OpenCV    written by Wesley T. Honeycutt" % ( version ))
...
# WTH disabled the auto add config function and just force quit at failure.
...
# WTH disabled search_config for now
from config import *
